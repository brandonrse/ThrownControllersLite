...
(function (objects) {
    var EasyQuestion = /** @class */ (function (_super) {
        ...
        EasyQuestion.prototype.loadEasyQuestions = function () {
            ...
            this.questions.push(new objects.Question("What is the name of the ring Master Chief crashes on in the first Halo game?", ["HALO", "THE HALO", "INSTALLATION 04", "INSTALLATION 4"], "Master Chief crashes onto Halo, also known as Installation 04, in the first Halo game."));
        };
        EasyQuestion.prototype.loadBrutalQuestions = function () {
            ...
            this.questions.push(new objects.Question("In Psychonauts, there is an achievement awarded for purchasing an item in the shop. More specifically, and absurdly, it must be purchased on a specific day of the year. What is the name of the achievement?", ["CHRISTMAS SHOPPING"], "Christmas Shopping is an achiement that can only be obtained on Christmas Day. Of course, this can be circumvented by simply changing the system's date."));
            this.questions.push(new objects.Question("Dedf1sh is the musician for Splatoon 2: Octo Expansion’s music. Some of the music she did is missing in the tracks' number order. Name one of the numbers of the tracks that are missing.", ["3", "7", "10", "15", "17", "18"], "The tracks that are missing are 3, 7, 10, 15, 17, and 18. When adding the numbers from the title of each song and then dividing the total by the number of songs Dedf1sh has composed (in other words, taking the average of the remaining numbers plus an extra 0 to represent Splattack!), the result is eight, the recurring theme of the Octo Expansion."));
            this.questions.push(new objects.Question("In Minecraft, there is a weather phenomenon known as thunderstorms where lightning can strike at any moment. When lightning strikes an entity, different things can happen depending on the entity. What item do turtles drop if struck by lightning?", ["BOWL", "BOWLS"], "Turtles upon being struck by lightning will instantly die, and anything that they would have dropped will be a bowl instead."));
            this.questions.push(new objects.Question("Super Smash Bros. Ultimate had a story that involved facing fighters with properties of 'things' from many game series, referred to as Spirits. Defeating them grants you that very same Spirit. How many Spirit battles are in Adventure Mode: World of Light?", ["936"], "There are a total of 936 Spirit Battles in Super Smash Bros. Ultimate's Adventure Mode: World of Light."));
        };
        EasyQuestion.prototype.loadTrickyQuestions = function () {
            ...
            this.questions.push(new objects.Question("In LEGO Star Wars: the Complete Saga, the characters must tell their story without the ability to speak. How does Darth Vader reveal to Luke that he is his father?", ["POINTING TO A PHOTO", "A PHOTO", "POINTING TO PHOTO", "POINTING A PHOTO", "SHOWS HIM A PHOTO", "SHOW HIM A PHOTO", "SHOW HIM PHOTO", "POINTS AT A PHOTO", "POINTS AT PHOTO", "POINTS TO A PHOTO", "POINTS TO A PICTURE"], "Darth Vader points to a photo of Anakin and Padme, and then to Luke. The message is conveyed."));
            this.questions.push(new objects.Question("The voice actor for the character Snatcher from A Hat In Time is a well-known internet personality. What is the name he goes by on the internet?", ["YUNGTOWN"], "Luke Sizemore aka Yungtown voice acts the ghostly being. He has also done a stream using Snatcher as a VTuber."));
            this.questions.push(new objects.Question("In Star Wars Jedi: Fallen Order, Cal Kestis must travel from planet to planet and evading galactic threats. What is the name of the ship?", ["MANTIS", "THE MANTIS"], "The Mantis is a pretty good ship to get around in. Perfect for running from the Empire."));
            this.questions.push(new objects.Question("Samus makes her return to Planet Zebes in Super Metroid pursuing Ridley. She uncovers the Space Pirates' new plot and destroys them all. Who is the final boss of Super Metroid?", ["MOTHER BRAIN", "MOTHERBRAIN"], "Mother Brain had been resuscitated by the Space Pirates after the events of Metroid and its remake Metroid: Zero Mission and is Samus's final opponent in Zebes."));
            this.questions.push(new objects.Question("Name one of the leaders of the Diamond or Pearl clan in Pokémon Legends: Arceus.", ["ADAMAN", "IRIDA", "ADAMAN AND IRIDA", "ADAMAN OR IRIDA"], "Adaman and Irida are relatively young leaders willing to put away their clan's pride if it meant saving the Noble Pokémon and all of Hisui."));
            this.questions.push(new objects.Question("The Second Sister is a reoccurring antagonist in Jedi: Fallen Order. What is her real name?", ["TRILLA", "TRILA"], "Trilla is the Inquisitor knows as the Second Sister."));
            this.questions.push(new objects.Question("What is the name of Mia Fey’s plant in the Ace Attorney series?", ["CHARLEY"], "Charley is Mia's beloved potted plant that has been taken care of over a decade by her protogé and her protogé's protogé."));
            this.questions.push(new objects.Question("How many Crystal Shards are in Kirby 64: The Crystal Shards?", ["74", "SEVENTY-FOUR", "SEVENTY FOUR"], "There are 74 Crystal Shards in Kirby 64: The Crystal Shards, 10 more Crystal Shards than you'd expect."));
        };
        return EasyQuestion;
    }(createjs.Container));
